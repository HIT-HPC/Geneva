...
void addExtPhase(uint svid1,uint extvid1,uint svid2,uint extvid2, uint sameEdgeLabelNum,
                 map<uint64_t,uint> &extendPhaseKeyCount, vector<PhaseSingleV>&matchOrder,map<uint,uint>&queryLabel){
    ...
    if(sameEdgeLabelNum==2){
        extphase2.pairs.push_back(svid2);
        extphase2.pairs.push_back(extvid2);
        ...
        matchOrder.push_back(extphase2);
    }
    
}
void addReducePhase(vector<uint> &pairs,map<uint64_t,phaseKeyCount> &reducePhaseKeyCount,
                    map<uint64_t,uint> &othersKeyCount,vector<PhaseSingleV>&matchOrder,map<uint,uint>&queryLabel){

    for(uint i=0;i<pairs.size()/2;++i){
        PhaseSingleV reducephase;
        ...
        reducephase.reducePos.push_back(1);
...
void genRestriction(vector<vector<uint>> automorphisms, map<uint,uint>& restricts,vector<PhaseSingleV>&matchOrder){
    vector<bool> isVisited(vnum,false);
    for(int i=0;i<matchOrder.size();++i){
        PhaseSingleV &tmpPhase = matchOrder[i];
...
void rearrangeReducePhase(map<uint64_t,uint>&extendPhaseKeyCount, map<uint64_t,uint>&othersKeyCount,map<uint64_t,phaseKeyCount>&reducePhaseKeyCount,
                          map<uint64_t,uint>&repeatFlag,vector<PhaseSingleV>&matchOrder,map<uint,map<uint,bool>>&recordLabelStat,map<uint,uint>&queryLabel){
    ...
    for(uint j=0;j<itercount;++j) {
        ...
        for (uint i = 0; i < phaselist.size() / 2; ++i) {
            ...
            PhaseSingleV &phase = matchOrder[phaseIndex];
...
void genMatchOrderSingleV(string queryFileName, vector<PhaseSingleV>&matchOrder, map<uint64_t,uint> &repeatFlag,map<uint,uint>&restricts,
    ...
    if(triangleCount[0].triCount==0){
        ...
        PhaseSingleV extphase1, extphase2, reducephase3;
